...
export const createCard = async (cardData: { accountId: string; type: 'debit' | 'credit'; pin: string; creditLimit?: number }): Promise<Card> => {
...
export const getCardById = async (cardId: string): Promise<Omit<Card, 'pinHash'> | undefined> => {
...
export const activateCard = async (cardId: string, pin: string): Promise<boolean> => {
  const card = await store.getById('cards', cardId);
...
export const changePin = async (cardId: string, oldPin: string, newPin: string): Promise<boolean> => {
  const card = await store.getById('cards', cardId);
